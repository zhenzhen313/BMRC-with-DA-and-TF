...
def calculate_entity_loss(pred, gold, gpu):
    pred = normalize_size(pred)
    gold = normalize_size(gold)

    weight = torch.tensor([1, 3.5, 3.5, 3.5]).float()
    if gpu:
        weight = weight.cuda()
    return F.cross_entropy(pred, gold.long(), reduction='sum', weight=weight, ignore_index=-1)


def calculate_loss(pred, gold):
    return F.cross_entropy(pred, gold.long(), reduction='sum', ignore_index=-1)
