...
import random
import sys
sys.path.append("/root/kaven/masked_t2")
...
inference_beta = [0.98, 0.98, 0.9, 0.98]
...
def test(model, tokenize, batch_generator, test_data, beta, logger, gpu, max_len):
    ...
    for batch_index, batch_dict in enumerate(batch_generator):
        ...
        ok_start_index = batch_dict['forward_asp_answer'][0].gt(-1).float().nonzero()

        ok_start_tokens = batch_dict['forward_asp_query'][0][ok_start_index].squeeze(1)

        f_asp_scores = model(batch_dict['forward_asp_query'],
                                                     batch_dict['forward_asp_query_mask'],
                                                     batch_dict['forward_asp_query_seg'], 'A')

        f_asp_scores = F.softmax(f_asp_scores[0], dim=1)

        f_asp_prob, f_asp_ind = torch.max(f_asp_scores, dim=1)

        f_asp_start_prob_temp = []
        f_asp_end_prob_temp = []
        f_asp_start_index_temp = []
        f_asp_end_index_temp = []
        for start_index in range(f_asp_ind.size(0)):
            if batch_dict['forward_asp_answer'][0, start_index] != -1:
                if f_asp_ind[start_index].item() == 1 or f_asp_ind[start_index].item() == 3:
                    f_asp_start_index_temp.append(start_index)
                    f_asp_start_prob_temp.append(f_asp_prob[start_index].item())
                if f_asp_ind[start_index].item() == 2 or f_asp_ind[start_index].item() == 3:
                    f_asp_end_index_temp.append(start_index)
                    f_asp_end_prob_temp.append(f_asp_prob[start_index].item())
        ...
        for start_index in range(len(f_asp_start_index)):
            opinion_query = tokenize.convert_tokens_to_ids(
                [word.lower() if word not in ['[CLS]', '[SEP]'] else word for word in
                 '[CLS] What opinions given the aspect'.split(' ')])
            ...
            f_opi_scores = model(opinion_query, opinion_query_mask, opinion_query_seg, 'AO')

            f_opi_scores = F.softmax(f_opi_scores[0], dim=1)
            f_opi_prob, f_opi_ind = torch.max(f_opi_scores, dim=1)

            f_opi_start_prob_temp = []
            f_opi_end_prob_temp = []
            f_opi_start_index_temp = []
            f_opi_end_index_temp = []
            for k in range(f_opi_ind.size(0)):
                if opinion_query_seg[0, k] == 1:
                    if f_opi_ind[k].item() == 1 or f_opi_ind[k].item() == 3:
                        f_opi_start_index_temp.append(k)
                        f_opi_start_prob_temp.append(f_opi_prob[k].item())
                    if f_opi_ind[k].item() == 2 or f_opi_ind[k].item() == 3:
                        f_opi_end_index_temp.append(k)
                        f_opi_end_prob_temp.append(f_opi_prob[k].item())
        ...
        b_opi_scores = model(batch_dict['backward_opi_query'],
                                                     batch_dict['backward_opi_query_mask'],
                                                     batch_dict['backward_opi_query_seg'], 'O')
        b_opi_scores = F.softmax(b_opi_scores[0], dim=1)
        b_opi_prob, b_opi_ind = torch.max(b_opi_scores, dim=1)

        b_opi_start_prob_temp = []
        b_opi_end_prob_temp = []
        b_opi_start_index_temp = []
        b_opi_end_index_temp = []
        for start_index in range(b_opi_ind.size(0)):
            if batch_dict['backward_opi_answer'][0, start_index] != -1:
                if b_opi_ind[start_index].item() == 1 or b_opi_ind[start_index].item() == 3:
                    b_opi_start_index_temp.append(start_index)
                    b_opi_start_prob_temp.append(b_opi_prob[start_index].item())
                if b_opi_ind[start_index].item() == 2 or b_opi_ind[start_index].item() == 3:
                    b_opi_end_index_temp.append(start_index)
                    b_opi_end_prob_temp.append(b_opi_prob[start_index].item())
        ...
        for start_index in range(len(b_opi_start_index)):
            aspect_query = tokenize.convert_tokens_to_ids(
                [word.lower() if word not in ['[CLS]', '[SEP]'] else word for word in
                 '[CLS] What aspects does the opinion'.split(' ')])
            ...
            b_asp_scores = model(aspect_query, aspect_query_mask, aspect_query_seg, 'OA')

            b_asp_scores = F.softmax(b_asp_scores[0], dim=1)
            b_asp_prob, b_asp_ind = torch.max(b_asp_scores, dim=1)

            b_asp_start_prob_temp = []
            b_asp_end_prob_temp = []
            b_asp_start_index_temp = []
            b_asp_end_index_temp = []
            for k in range(b_asp_ind.size(0)):
                if aspect_query_seg[0, k] == 1:
                    if b_asp_ind[k].item() == 1 or b_asp_ind[k].item() == 3:
                        b_asp_start_index_temp.append(k)
                        b_asp_start_prob_temp.append(b_asp_prob[k].item())
                    if b_asp_ind[k].item() == 2 or b_asp_ind[k].item() == 3:
                        b_asp_end_index_temp.append(k)
                        b_asp_end_prob_temp.append(b_asp_prob[k].item())
        ...
        for idx in range(len(final_asp_list)):
            ...
            for idy in range(predict_opinion_num):
                ...
                sentiment_query = torch.cat([sentiment_query, ok_start_tokens], -1)
                sentiment_query_seg += [1] * ok_start_tokens.size(0)
                sentiment_query_mask = [1] * sentiment_query.size(0)

                if sentiment_query.size(0) < 128:
                    length = 128 - sentiment_query.size(0)
                    sentiment_query_add = torch.tensor([tokenize.convert_tokens_to_ids('[PAD]')] * length).long()
                    if gpu:
                        sentiment_query_add = sentiment_query_add.cuda()
                    sentiment_query = torch.cat([sentiment_query, sentiment_query_add], -1)
                    sentiment_query_seg += [1] * length
                    sentiment_query_mask += [0] * length

                if gpu:
                    sentiment_query_mask = torch.tensor(sentiment_query_mask).float().cuda()
                    sentiment_query_seg = torch.tensor(sentiment_query_seg).long().cuda()
                sentiment_query = sentiment_query.unsqueeze(0)
                sentiment_query_mask = sentiment_query_mask.unsqueeze(0)
                sentiment_query_seg = sentiment_query_seg.unsqueeze(0)
...
def train(arguments):
    ...
    # tokenize = AutoTokenizer.from_pretrained(arguments.bert_model_type)
    # load data
    ...
    if arguments.mode == 'test':
        ...
        for epoch in range(start_epoch, arguments.epoch_num + 1):
            ...
            for batch_index, batch_dict in enumerate(batch_generator):
                ...
                f_aspect_scores = model(batch_dict['forward_asp_query'],
                                                                   batch_dict['forward_asp_query_mask'],
                                                                   batch_dict['forward_asp_query_seg'], 'A')

                f_asp_loss = Utils.calculate_entity_loss(f_aspect_scores, batch_dict['forward_asp_answer'], arguments.gpu)

                b_opi_scores = model(batch_dict['backward_opi_query'],
                                                             batch_dict['backward_opi_query_mask'],
                                                             batch_dict['backward_opi_query_seg'], 'O')

                b_opi_loss = Utils.calculate_entity_loss(b_opi_scores, batch_dict['backward_opi_answer'], arguments.gpu)
                ...
                f_opi_loss = Utils.calculate_entity_loss(
                    f_opi_scores,
                    batch_dict['forward_opi_answer'].view(-1, batch_dict['forward_opi_answer'].size(-1)),
                    arguments.gpu
                ) / max_aspect_num
                ...
                b_asp_loss = Utils.calculate_entity_loss(
                    b_asp_scores,
                    batch_dict['backward_asp_answer'].view(-1, batch_dict['backward_asp_answer'].size(-1)),
                    arguments.gpu
                ) / max_aspect_num
                ...
                sentiment_loss = Utils.calculate_loss(
                    sentiment_scores,
            ...
            batch_generator_dev = Data.generate_batches(dataset=dev_dataset, batch_size=1, shuffle=False,
                                                        gpu=arguments.gpu)
            logger.info("dev")
            f1_test = test(model, tokenize, batch_generator_dev, dev_standard, arguments.inference_beta, logger,
                 arguments.gpu, max_len)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--warm_up', type=float, default=0.05)
